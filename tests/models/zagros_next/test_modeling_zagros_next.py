...
class ZagrosNextModelTester:
    def __init__(
        ...
        self.linear_conv_kernel_dim = 3
        self.layer_types = ["full_attention", "linear_attention", "full_attention", "linear_attention"]
    ...
    def create_and_check_model(
        ...
        model = ZagrosNextModel(config=config)
    ...
    def create_and_check_for_causal_lm(
        ...
        model = ZagrosNextForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class ZagrosNextModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (ZagrosNextModel, ZagrosNextForCausalLM)  # Add other classes if needed
    pipeline_model_mapping = {
        "feature-extraction": ZagrosNextModel,
        "text-generation": ZagrosNextForCausalLM,
    }
    fx_compatible = True
    ...
    def setUp(self):
        self.model_tester = ZagrosNextModelTester(self)
        self.config_tester = ConfigTester(self, config_class=ZagrosNextConfig, hidden_size=37)
